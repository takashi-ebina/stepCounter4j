 ...
 * このクラスを継承し、{@link AbsStepCount#fileReadStepCount(File)}をオーバーライドして、個別に実装してください。<br>
...
public abstract class AbsStepCount implements IfStepCount {
	...
	public StepCountData stepCount(final File inputFile) {
		return fileReadStepCount(inputFile);
	}
	 ...
	 * @return 1ファイル単位の総行数／実行行数／コメント行数／空行数を集計するデータクラス
	 */
	abstract protected StepCountData fileReadStepCount(final File inputFile);
}