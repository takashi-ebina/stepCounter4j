...
public class JavaStepCount extends AbsStepCount {
	 ...
	 * <p>
	 * 引数のカウント対象プログラムファイルを元に、総行数／実行行数／コメント行数／空行数を集計処理を行います。
	 * 
	 * @param inputFile inputFile ステップカウント対象ファイル
	 * @return 1ファイル単位の総行数／実行行数／コメント行数／空行数を集計するデータクラス
	 */
	@Override
	protected StepCountData fileReadStepCount(final File inputFile) {
		...
		try (BufferedReader bw = new BufferedReader(
			...
			logger.logError("Javaステップ数集計処理で例外発生。 ファイル名：" + inputFile.getName(), e);
